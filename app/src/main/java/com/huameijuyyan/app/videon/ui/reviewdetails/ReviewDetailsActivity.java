...
import com.huameijuyyan.app.util.Loader;
import com.huameijuyyan.app.util.NetworkHelper;
import com.huameijuyyan.app.util.helper.Constants;
import com.huameijuyyan.app.R;
import com.huameijuyyan.app.videon.data.local.response.FeedBackModel;
import com.huameijuyyan.app.videon.data.local.response.FeedBackResponse;
import com.huameijuyyan.app.videon.data.local.response.ReviewImage;
import com.huameijuyyan.app.videon.data.local.response.StartRatingModel;
...
import com.huameijuyyan.app.databinding.ActivityReviewDetailsBinding;
...
public class ReviewDetailsActivity extends BaseActivity {
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.button_retry) {
            mLoader.show();
            showReviewFromServer(this, itemId);
        }
    }
...
/*    private void openDialog() {
    ...
    private void getIntentDataAndHitToServer() {
        ...
        AsyncTask.execute(new Runnable() {
            @Override
            public void run() {
                showReviewFromServer(ReviewDetailsActivity.this, itemId);
            }
        });
    }
    ...
    public void showReviewFromServer(Context context,String itemId){
        ...
        if (NetworkHelper.hasNetworkAccess(context)) {
            RemoteVideoApiInterface mRemoteLoginLogoutApiInterface =  RemoteApiProvider.getInstance().getRemoteHomeVideoApi();
            Call<FeedBackResponse> call=mRemoteLoginLogoutApiInterface.showReview(Constants.ServerUrl.API_TOKEN, itemId);
                ...
                public void onResponse(@NonNull Call<FeedBackResponse> call, @NonNull Response<FeedBackResponse> response) {
                    if (response.isSuccessful() && response.body() != null){
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                if (feedBackResponse.getFeedBackModel() != null) {
                                    if (feedBackResponse.getFeedBackModel().getReviewImageList().isEmpty()) {
                                        mBinding.layoutHeader.setVisibility(View.GONE);
                                        mBinding.layoutNoData.setVisibility(View.VISIBLE);
                                    } else {
                                        mAdapter.clearList();
                                        mAdapter.addItem(feedBackResponse.getFeedBackModel().getReviewImageList());
                                        mReviewImageList = feedBackResponse.getFeedBackModel().getReviewImageList();
                                        mBinding.layoutHeader.setVisibility(View.VISIBLE);
                                        mBinding.layoutNoData.setVisibility(View.GONE);
                                    }
                                    setData(feedBackResponse.getFeedBackModel());
                                    mBinding.layoutIncludeNoNet.linearNoInternet.setVisibility(View.GONE);
                                    mBinding.scrollView.setVisibility(View.VISIBLE);
                                } else {
                                    mBinding.layoutHeader.setVisibility(View.GONE);
